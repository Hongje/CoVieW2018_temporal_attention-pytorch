...
class Temporal_Attention_bias_ver2(nn.Module):
    def __init__(self, data_length=[1024,128], class_length=[29,285], only_scene=False, only_action=False,
                 temporal_bias_init = None):
        super(Temporal_Attention_bias_ver2, self).__init__()
        if only_scene:
            out_num = class_length[0]
        elif only_action:
            out_num = class_length[1]
...
class Temporal_Attention_ver2_softmax(nn.Module):
    def __init__(self, data_length=[1024,128], class_length=[29,285], only_scene=False, only_action=False):
        super(Temporal_Attention_ver2_softmax, self).__init__()
        if only_scene:
            out_num = class_length[0]
        elif only_action:
            out_num = class_length[1]
    ...
    def forward(self, x):
        ...
        T_attention = self.attention(out)
        T_attention = F.softmax(T_attention, dim=2)
        T_attention_expand = T_attention.expand_as(out)
...
class Temporal_Attention_bias_ver2_softmax(nn.Module):
    def __init__(self, data_length=[1024,128], class_length=[29,285], only_scene=False, only_action=False,
                 temporal_bias_init = None):
        super(Temporal_Attention_bias_ver2_softmax, self).__init__()
        if only_scene:
            out_num = class_length[0]
        elif only_action:
            out_num = class_length[1]
